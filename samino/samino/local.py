...
class Local:
    ...
    def get_chat_threads(self, start: int = 0, size: int = 25):
        req = requests.get(f'{self.api}/x{self.comId}/s/chat/thread?type=joined-me&start={start}&size={size}',
                           headers=self.headers)
        if req.status_code != 200:
            return CheckExceptions(req.json())
        else:
            return MyChats(req.json())

    def get_member_visitors(self, userId: str, start: int = 0, size: int = 25):
        req = requests.get(f'{self.api}/x{self.comId}/s/user-profile/{userId}/visitors?start={start}&size={size}',
                           headers=self.headers)
    ...
    def get_chat_messages(self, chatId: str, size: int = 25):
        req = requests.get(f'{self.api}/x{self.comId}/s/chat/thread/{chatId}/message?v=2&pagingType=t&size={size}',
                           headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
    ...
    def get_wall_comments(self, userId: str, sorting: str,
        ...
        req = requests.get(
            f"{self.api}/x{self.comId}/s/user-profile/{userId}/g-comment?sort={sorting}&start={start}&size={size}",
            headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        return Comment(req.json()['commentList'])

    def get_all_users(self, start: int = 0, size: int = 25):
        req = requests.get(f'{self.api}/x{self.comId}/s/user-profile?type=recent&start={start}&size={size}',
                           headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        return UserList(req.json()['userProfileList'])
    ...
    def send_message(self, chatId: str, message: str = None, messageType: int = 0, replyTo: str = None,
                     ...
                     embedTitle: str = None, embedContent: str = None, isWeb: bool = False):
        if isWeb:
            data = {
    ...
    def unfollow(self, userId: str, isWeb: bool = False):
        if isWeb:
            data = {
                "ndcId": f"x{self.comId}",
                "followee_id": userId,
    ...
    def follow(self, userId: [str, list], isWeb: bool = False):
        if isWeb:
            data = {
                "ndcId": f"x{self.comId}",
                "followee_id": userId
        ...
        if isinstance(userId,
                      list): api = f'{self.api}/x{self.comId}/s/user-profile/{self.uid}/joined'; data = json.dumps(
    ...
    def vote_comment(self, blogId: str, commentId: str, value: int = 1):
        ...
        if value != 1:
            ...
            raise TypeError(f"Please fill the value by (-1 or 1) not ({value})")
        data = json.dumps({"value": value})
        req = requests.post(f"{self.api}/x{self.comId}/s/blog/{blogId}/comment/{commentId}/vote?cv=1.2&value=1",
                            data=data, headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        return Json(req.json())
    ...
    def delete_message(self, chatId: str, messageId: str):
        req = requests.delete(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/message/{messageId}",
                              headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
        return Json(req.json())
    ...
    def edit_blog(self, title: str, content: str, blogId: str = None, wikiId: str = None, fansOnly: bool = False,
                  background: BinaryIO = None, backgroundColor: str = None):
        data = {
            "title": title,
            "content": content
        }
    ...
    def get_chat_bubbles(self, start: int = 0, size: int = 20):
        req = requests.get(
            url=f"{self.api}/x{self.comId}/s/chat/chat-bubble?type=all-my-bubbles&start={start}&size={size}",
            headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
