...
class GlobalClient:
    ...
    def login(self, email: str, password: str):
        ...
        return objects.LoginInfo(response=request)

    def send_message(self, chatId: str, message: str, clientRefId: int = pack.timestamp()//1000, type: int = 0):
        data = {
            'content': message,
            'type': type,
    ...
    def join_chat(self, chatId: str):
        r = requests.post(url=f'{self.api}g{self.chat + chatId}/member/' + self.uid['uid'], headers=self.headers).text
        request = json.loads(r)
        return objects.ApiMsg(response=request)

    def leave_chat(self, chatId: str):
        r = requests.delete(url=f'{self.api}g{self.chat + chatId}/member/' + self.uid['uid'], headers=self.headers).text
        request = json.loads(r)
        return objects.ApiMsg(response=request)
