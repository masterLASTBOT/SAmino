...
class LocalClient:
    ...
    def invite_by_host(self, chatId: str, userId: [str, list]):
        ...
        r = requests.post(f"http://service.narvii.com/api/v1/{self.comId}/s/chat/thread/{chatId}/avchat-members",headers=self.headers, data=data)
        request = json.loads(r.text)
        return objects.ApiMsg(request)
    ...
    def start_chat(self, userId: str, content: str):
        data = json.dumps({
            "type": 1,
            "inviteeself.uids": userId,
            "timestamp": pack.timestamp()
        })
        r = requests.post(f"http://service.narvii.com/api/v1/{self.comId}/s/chat/thread", headers=self.headers, data=data)
        request = json.loads(r.text)
        return objects.ApiMsg(request)
    ...
    def edit_chat(self, chatId: str, content: str, title: str):
        ...
        r = requests.post(f"http://service.narvii.com/api/v1/{self.comId}/s/chat/thread/{chatId}", headers=self.headers, data=data)
        request = json.loads(r.text)
        return objects.ApiMsg(request)
